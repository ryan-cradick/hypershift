...
func (r *HostedControlPlaneReconciler) reconcileDNSOperator(ctx context.Context, hcp *hyperv1.HostedControlPlane, releaseImageProvider *imageprovider.ReleaseImageProvider, userReleaseImageProvider *imageprovider.ReleaseImageProvider, createOrUpdate upsert.CreateOrUpdateFN) error {
	...
	// certificate chain
	util.Rlogger = r.Log
	util.Logloud("manifests.RootCAConfigMap(hcp.Namespace)", lastRootCA, fmt.Sprint(rootCA.Data))
	...
	util.Logloud("manifests.CSRSignerCASecret(hcp.Namespace)", lastCSRSigner, fmt.Sprint(csrSigner))
	...
	util.Logloud("reconciled ServiceAccountKubeconfig", lastkubeconfig, kubeconfigstr)
	...
	util.Logloud("reconciled dnsoperator deployment", lastDeployment, fmt.Sprint(deployment))
